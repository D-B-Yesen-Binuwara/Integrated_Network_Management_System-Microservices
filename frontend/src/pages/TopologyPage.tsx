...
export function TopologyPage() {
  ...
  return <>
    ...
    <div className="topology-records"><Panel title="Topology records" description="Device IP address and priority are supplied by the topology service."><RequestState loading={loading} error={error} empty={empty} onRetry={reload}><div className="table-wrap"><table className="data-table"><thead><tr><th>Device</th><th>Type</th><th>IP address</th><th>Priority</th><th>Status</th><th>Relationships</th></tr></thead><tbody>{(devices.data ?? []).map((device) => <tr key={device.deviceId}><td>{device.deviceName}</td><td>{device.deviceType}</td><td>{device.ip || 'Unavailable'}</td><td><span className={`priority-badge priority-${device.priorityLevel.toLowerCase()}`}>{device.priorityLevel}</span></td><td><span className={`status-badge ${statusTone(device.status)}`}>{device.status}</span></td><td className="muted-cell">{(links.data ?? []).filter((link) => link.parentDeviceId === device.deviceId || link.childDeviceId === device.deviceId).length}</td></tr>)}</tbody></table></div></RequestState></Panel></div>
  </>
}
